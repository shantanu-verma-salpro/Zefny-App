...
import { Skeleton, Button, Empty  } from "antd";
...
export default function GetPosts(props) {
  ...
  if (data.data.length === 0) return <div><Empty /><br /> <Button className={styles.nextbtn} type="primary" onClick={props.homeHandler}>Initial Page</Button></div>;
  return (
    <div>
      {data.data.map((m) => (
      ...
      <Button type="primary" className={styles.nextbtn} onClick={props.handler}>Next</Button>
    </div>
